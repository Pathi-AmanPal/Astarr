...
# The supervisory bands. The frontend reads the same two numbers; if they ever
# diverge, verify.py fails.
BAND_EXCEPTION = 50.0
BAND_CAUTION = 10.0
...
def get_distribution_metrics(con: duckdb.DuckDBPyConnection, by: str) -> dict:
    ...
    if by == "severity":
        ...
        # The supervisory bands, and the ONE place they are defined.
        #
        # These read >= 40 / >= 15 until 2026-09-06, while the ranking screen banded
        # at > 50 / >= 10. An entity scoring 45 was therefore "Exception" on the
        # Overview and "caution" on the very table that screen links to -- two answers
        # to one question, which is exactly what a supervisor notices and loses
        # confidence over.
        #
        # 50/10 wins because it is the pair with a recorded reason: it preserves the
        # Phase 1 three-band grouping exactly under weighted tiers, so no entity
        # silently changed band when the formula changed. Banding is a supervisory
        # judgement rather than arithmetic -- moving these is a product decision, and
        # moving them in one place only is a bug. verify.py asserts the two agree.
        bands = {
            "Exception (> 50)": 0,
            "Caution (10 - 50)": 0,
            "Clear (< 10)": 0,
        ...
        for e in entities:
            ...
            if score > BAND_EXCEPTION:
                bands["Exception (> 50)"] += 1
            elif score >= BAND_CAUTION:
                bands["Caution (10 - 50)"] += 1
            else:
                bands["Clear (< 10)"] += 1
        items = [
            {
